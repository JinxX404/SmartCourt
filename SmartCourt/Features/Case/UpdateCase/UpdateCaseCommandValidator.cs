...
public class UpdateCaseCommandValidator : AbstractValidator<UpdateCaseCommand>
{
    ...
    {
        RuleFor(c => c.CaseId).NotEmpty().WithMessage("Case id is required");
        RuleFor(c => c.Title).NotEmpty().WithMessage("Case title can't be empty");
        RuleFor(c => c.Description).NotEmpty().WithMessage("Case description can't be empty");
