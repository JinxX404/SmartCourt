...
| ID | When this happens | Actor → recipient | User story / message intent | Type | Priority / severity | Readiness | Suggested channel |
|---|---|---|---|---|---|---|---|
| `AUT-01` | An authenticated user changes their password and all refresh tokens are revoked. | User/System → Account owner | As the account owner, I want a security receipt and instructions if I did not make the change. | `security.password-changed` | `P1 / Critical` | `Implemented`; `PasswordChanged` V1 in password-change transaction | Existing Email receipt + in-app |
| `AUT-02` | A password reset succeeds and refresh tokens are revoked. | User/System → Account owner | As the account owner, I want a security receipt and recovery guidance if unauthorized. | `security.password-reset` | `P1 / Critical` | `Implemented`; `PasswordReset` V1 in reset transaction | Existing Email receipt + in-app |
...
### Implemented Gate 7 contract

`PasswordChanged` V1 is queued only after `ChangePasswordService` successfully changes the password and revokes active refresh tokens, before the existing transaction commits. `PasswordReset` V1 follows the same rule in `ResetPasswordService` after a valid reset token changes the password and active refresh tokens are revoked. The account owner is the only recipient, resolved by the Auth-owned authoritative context reader. Login, registration, confirmation, resend, forgot-password request, phone challenge, refresh, logout, ordinary token revoke, failed password operations, invalid reset tokens, and replayed reset tokens create no in-app security row.

The persisted contracts are `security.password-changed` (`Critical`, title `تم تغيير كلمة المرور`, body `تم تغيير كلمة مرور حسابك بنجاح. إذا لم تكن أنت من أجرى هذا التغيير، يرجى تأمين حسابك والتواصل مع الدعم.`) and `security.password-reset` (`Critical`, title `تمت إعادة تعيين كلمة المرور`, body `تمت إعادة تعيين كلمة مرور حسابك بنجاح. إذا لم تطلب هذا الإجراء، يرجى تأمين حسابك والتواصل مع الدعم.`). Both use `actionUrl: null` and data keys only `userId`. Email addresses, passwords/hints, reset URLs/tokens, access/refresh tokens, IP addresses, device fingerprints, security stamps, provider IDs, and idempotency keys are forbidden. Existing Email security receipts remain unchanged. Outbox-message replay is idempotent; REST is durable and SignalR is best-effort.

Gate 7 is verified by [`AuthSecurityNotifications_Report.md`](../../../SmartCourt.Tests/HttpTests/AuthSecurityNotifications_Report.md) with `117 passed, 0 failed, 1 documented skip`. The report covers every Auth endpoint, authentication boundaries, password-change/reset causal persistence, failure/replay/concurrency behavior, token revocation, exact Arabic payloads, forbidden fields, recipient isolation, mock Email link extraction, and clean API/outbox/provider monitoring.
